...
public interface BuildingMapper {
	...
	Long selectBuildingIdByUuidAndMajorAndMinor(@Param("uuid") String uuid, @Param("major") String major,
			@Param("minor") String minor);

	@Select("SELECT DISTINCT building.* FROM building_and_group, building, `group`, user"
			+ " WHERE building_and_group.building_id=building.id" + " AND building_and_group.group_id=group.id"
			+ " AND group.large_name=(" + " SELECT group.large_name" + " FROM user, `group`"
			+ " WHERE user.username=#{username}" + " AND user.group_id=group.id" + ")")
	List<Building> selectBuildingByUsername(@Param("username") String username);

	// 해당 유저가 속한 그룹의 대그룹에 속한 그룹들의 빌딩 Id 들을 가져옴
	@Select("SELECT * FROM building WHERE id" + " IN (SELECT building_id FROM building_and_group WHERE group_id"
			+ " IN (SELECT id FROM `group` WHERE large_name=" + "(SELECT large_name FROM `group` WHERE id="
